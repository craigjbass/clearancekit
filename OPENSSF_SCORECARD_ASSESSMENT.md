...
| **Branch-Protection** | 3/10 | High | Partially mitigated | PR-based merge requirements are not applicable (trunk-based dev). Remaining actionable controls: disallow force-pushes to `main`, disallow branch deletion, require signed commits. These can be enabled in GitHub branch protection without requiring PR reviews. |
